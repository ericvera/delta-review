...
#### Body budget & the full-note document

A thread is a `ZoneWidget` whose height VS Code clamps to `max(12, editorHeight / lineHeight * 0.8)` lines with `.review-widget .body { overflow: hidden }` — past the clamp, content is clipped with **no scrollbar at any level**, and no extension API reaches the clamp or that CSS. Three things keep notes usable inside it:

- **Height tracks the text.** `escapeMarkdownText` (`src/markdown.ts`) emits markdown hard line breaks (two trailing spaces), so a note is one `<p>` rather than one per line — `.comment-body p { margin: 8px 0 }` used to add ~40%. A source blank line becomes a spaces-only line, still blank in CommonMark, so paragraph breaks survive; joining lines is why `=`, `|` and `<` are escaped (setext heading, GFM table, HTML block — each needs a second line to form, and the block that forms eats the lines it spans).
- **Each rendered turn is budgeted** (`src/noteBody.ts`, constants are the tuning knobs): a 24-line thread total gives a per-turn budget of `max(4, 24 / turnCount)` capped at 10 lines and 600 characters. Deriving it from the turn count is what bounds a many-turn thread; past the floor it only bounds the common cases, which is why the title-bar button below is unconditional. Truncation is presentation only — stored notes are untouched, and editing a truncated turn still opens its full raw text (`NoteComment.turnText`).
- **Escape hatches.** A truncated turn carries an inline `Show full note` command link (bodies are `isTrusted` scoped to that one command); every thread carries an **Open Full Note** title-bar button, which renders above the clipped region and therefore can never itself be clipped. Both open the whole thread — heading, status, every turn with author and timestamp — as a read-only virtual markdown document under the `delta-review-note` scheme (`src/noteContentProvider.ts`, note id in the query, text from `src/noteBody.ts`), which scrolls natively. `renderNoteThreads` fires the provider's change event for open note documents, so an agent reply updates an open tab.
...
41. Long note: add a note whose text runs well past 10 lines → the thread renders complete-but-truncated (no text clipped mid-widget) with a `Show full note` link, and the title bar's **Open Full Note** ($(open-preview)) — present on open, addressed and resolved threads alike — opens a scrollable read-only document with every turn, author and timestamp. Edit the truncated turn → the editor opens on the **full** raw text and saving preserves it. Append an agent response while that document is open → it updates in place. Blank lines inside a note still separate paragraphs, and long lines still wrap rather than scrolling sideways.
