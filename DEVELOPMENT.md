...
`−` is the inverse of a mark, and it selects by **snapshot presence, never by status**:

- Every unmark — file row, folder, cluster, Auto subgroup, section header — drops the snapshot of every path in its scope whatever that path currently reads as, so a file whose content diverged from its snapshot is one press away from the merge-base diff again. That is also why `−` renders wherever the scope holds a snapshot at all, needs-review rows included.
- The **Reviewed** header's `−` is scoped to the branch, not to the review set: it writes an empty state (`clearAllReviewSnapshots`, `src/reviewState.ts`), so snapshots for paths that have since dropped out of the review set go too, and the ref keeps its history. The palette's **Clear Review State** is the blunter tool — it deletes the ref outright, behind a modal.
- Rows advertise that scope through their contextValue. `src/rowContext.ts` (pure, so the rule stays testable without `treeProvider`'s `vscode` import) appends `Snapshot` to the needs-review file, folder and Auto values — `needsReviewFileSnapshot`, `needsReviewFileDeletedSnapshot`, `needsReviewFolderSnapshot`, `needsReviewAutoGroupSnapshot` — and `clusterContextValue` (`src/clusters.ts`) returns `clusterNeedsReviewSnapshot`. Tokens append in a fixed order — status, then `Deleted`, then `Snapshot` — so the `Deleted` matches that hide **Open File** keep lining up. Reviewed rows carry no token: they always have a snapshot.

Deriving that status means content-hashing the working tree, which is what makes a refresh expensive on a large change. Two things bound it (`src/hashCache.ts`, `computeReviewModel` in `src/model.ts`):

- **Hash cache** — one `Map` per extension lifetime, path → `{ size, mtimeMs, sha }`. A refresh stats the review set and only sends the paths whose size or mtime moved to `git hash-object --stdin-paths`; a path with no stat vanished mid-refresh and is dropped rather than failing the whole batch. Both `computeReviewModel` calls in a `refresh()` share the instance, so the auto-mark recomputation re-hashes nothing. It is cleared on repository switch only — content identity is not branch-scoped — and a same-size write inside one mtime tick can leave a stale sha, the window git itself lives with; the next save's refresh heals it.
- **Scoped merge-base lookup** — `ls-tree` is limited to the paths the model can ask about (the review set, rename/move origins, declared destinations), batched at 500 pathspecs and sent with `:(literal)` magic so a filename containing `*`, `?` or `[` still matches. A pathspec that escapes the repo (an external move origin) is never sent.

Mark and unmark run inside `vscode.window.withProgress` on the view, so the queue wait, the ref write and the refresh that follows all read as busy rather than as a dead click.

Every write to the ref — inline `+`/`−`, the bulk and cluster actions, auto-marking, and Clear Review State — is serialized through one in-process queue (`src/serialQueue.ts`). Each mutation reads the ref, changes the path→blob map, and writes it back, so two overlapping writes would silently drop one side's paths. Only the mutation is queued, never the `refresh()` that follows it: refresh's auto-mark pass enqueues a write of its own and would deadlock behind a caller still holding the slot. A mark/unmark command that fails anywhere — the git write or that refresh — reports it as an error notification carrying git's stderr; the watcher-driven background refresh stays silent (it surfaces failures in the view's message banner as before).
...
Inline notes on diff lines, threaded with an agent's replies. Three files per branch under `<git common dir>/delta-review/` (same dir and branch sanitization as clusters):
...
- `archive-<sanitized branch>.json` — **extension-owned, append-only history**: Clear Resolved appends every note it removed, each the note object as it stood plus `deletedAt`, right after the notes-file rewrite and before but independently of re-anchoring (`archiveNotes` in `src/notesStore.ts`; shell parser `parseArchiveFile` in `src/notes.ts` validates only `version` and `notes`, since entries are opaque history). The extension never reads it for behavior of its own — only as the read half of the append. Unparsable → renamed aside to `<file>.corrupt-<stamp>` and a fresh archive started; unreadable or unwritable → nothing is appended and the existing file is left as it was. Either way a warning toast names what happened (`archiveWarning`) and the clear itself stands. Explicit deletes (thread or row Delete, only-turn deletion) never archive.

Types and whole-file parsers live in `src/notes.ts` (clusters semantics for the notes and responses files: one violating entry rejects the file with a one-line error); persistence and mutation in `src/notesStore.ts` (atomic same-dir temp+rename saves, an idempotence guard so identical saves never touch the file — no watcher loops — and load→modify→save helpers).

#### The ref: `refs/review-notes/<branch>`

Each note snapshots the whole noted document as a git blob (`contentBlob`, via `hash-object -w`). All live blobs are anchored by a commit on `refs/review-notes/<branch>` (tree path = note id → blob), so `git gc` cannot prune them. It is deliberately separate from `refs/review/<branch>`: Clear Review State must not destroy note snapshots. The ref is deleted when the last note goes. Notes removed by Clear Resolved drop out of the tree, so their blobs are no longer kept alive and `git gc` may prune them — the archive's `snapshot` is the surviving text record. Inspect with `git ls-tree refs/review-notes/<branch>`.
...
Rendering is the standard VS Code comments API (`src/commentController.ts`) — threads appear in the built-in Comments panel for free, nothing is built against it. The REVIEW NOTES section (`src/notesTreeProvider.ts`) is a sibling SCM view fed the same merged threads; `renderNoteThreads` in `src/extension.ts` fans out to both surfaces plus the view badge. Each note row also carries Resolve / Unresolve / Delete (Delete confirms first), which go straight to `src/notesStore.ts` — no diff and no rendered thread involved, so a note stays closable whatever happened to its file.

Two rendering invariants sit behind that, with their decisions in the vscode-free `src/threadRender.ts` so they are unit-testable:

- **A thread's reply box appears only on a freshly created thread.** VS Code builds a second reply button when an extension flips a live thread's `canReply` false→true (the lazily built form's async init and the accompanying thread update each create one), so `restyle` disposes and recreates the thread — with its comments, so the widget builds its form once at display. That drops an in-progress turn edit on that thread; accepted, since the typed text lives in the disposed widget either way.
- **`styleThread` writes only changed fields.** Every `comments`/`label`/`contextValue`/`state` write emits an update that could land on a form still initializing. Comment identity includes the **body kind** (raw string vs `MarkdownString`): handlers mutate comment objects in place, so the raw string an edit leaves behind is the only marker that the display body is owed a re-send.

#### Where a note's thread lives

`src/noteTarget.ts` (pure) decides the document a note attaches to, from `(model, note, workingFileExists)`; the thread and the REVIEW NOTES click both resolve through it, so the click always lands on the document the thread is on. Five cases:

- **Base-side note, any file** — the base document named by [Base document identity](#base-document-identity), falling back to the note's `contentBlob`.
- **Working side, file in the review set, not deleted** — the working file.
- **Working side, file in the review set, deleted** — that file's base document at `diffBaseSha ?? note.contentBlob`, never the empty document: a deleted file's diff has an empty right side, so only the base holds the noted content.
- **Working side, file absent from the review set, present on disk** — the working file; a real, editable file beats a historical snapshot.
- **Working side, file absent from both** — the note's own `contentBlob`, addressed in that blob's **creation** coordinates (`startLine`/`endLine`) rather than the working-tree-drifted `current*` pair.

`reviewDiffIsEmpty` (same module) is the click path's escape hatch: a deleted file with no base blob diffs empty against empty, so instead of opening that diff the click opens the resolved target document directly.

An anchor can sit past the end of its target document, and clamping it needs a line count, which only exists once the document is open — so `src/extension.ts` re-clamps on `onDidOpenTextDocument` (file and `delta-review-base` schemes only). That listener must never call `refresh()` (git on every document open) or `renderThreads` (it disposes any cached thread absent from the array it is given, including the one `addNote` adopts before its refresh lands); it calls the controller's range-only `reclampThreadsFor`.

#### Long notes

Turns render in full: `commentsFor` (`src/commentController.ts`) puts `escapeMarkdownText(turn.text)` in the comment body verbatim, with no budget, truncation or escape hatch. Editing a turn swaps in its raw text (`NoteComment.turnText`).

- **Height tracks the text.** `escapeMarkdownText` (`src/markdown.ts`) emits markdown hard line breaks (two trailing spaces), so a note is one `<p>` rather than one per line — `.comment-body p { margin: 8px 0 }` would otherwise add ~40%. A source blank line becomes a spaces-only line, still blank in CommonMark, so paragraph breaks survive; joining lines is why `=`, `|` and `<` are escaped (setext heading, GFM table, HTML block — each needs a second line to form, and the block that forms eats the lines it spans).
- **VS Code sizes the thread widget to its content** (≥ 1.98): the comment thread relayouts its zone to the measured content height without the zone widget's `max(12, 0.8 × editor lines)` cap, so a long thread grows past the viewport and the editor scrolls. The only remaining limit is per comment body — `max-height: 20em` with its own scrollbar, applied at comment-node construction while `comments.maxHeight` is not `false`.
- **The extension contributes `"comments.maxHeight": false`** (`contributes.configurationDefaults`, `package.json`) so the reviewer scrolls one thing, the editor. It is a window-wide default covering every comment provider (GitHub PR comments included); an explicit user setting overrides it.
...
4. Edit the file → it moves back to Needs Review, and its diff is now _last reviewed ↔ working tree_ (only the new edit). The row now offers `−` alongside `+`: press `−` → the snapshot is dropped and the diff is _merge base ↔ working tree_ again.
5. `+` to snapshot it again, edit it once more, then revert that edit (undo + save) → content matches the snapshot again, file returns to Reviewed on its own.
6. Commit / rebase — review state is unaffected (content-based).
7. Tree/list toggle, folder `+`/`−` (a Needs Review folder offers `−` too, once a child holds a snapshot), collapse state surviving reload, and repo switching in Source Control all behave as before.
8. Branch-wide reset: mark several files reviewed, then edit each of them → they all read Needs Review with delta diffs, and the **Reviewed** header shows `0` while still offering `−`. Press it → `git ls-tree -r refs/review/<branch>` prints nothing, every one of those files diffs _merge base ↔ working tree_ again, and `git log refs/review/<branch>` still lists the earlier sessions under the empty state.

Moves:

9. `git mv` a changed-or-unchanged file to another directory (pure move) → one **R** row at the new path, no row at the old path. Description `<new dir> ← <old dir> · verbatim`, with the origin's trailing segments shared with the new path dropped (in tree mode there is no directory text and the row starts at `←`); tooltip `Moved from <full old path>` then, a blank line later, `Identical to the origin`. Its diff says the files are identical, the title reads `<name> (moved from <old path> — merge base ↔ working tree)`, and the left editor is labeled with the old path. That last part holds for a repo origin only while the destination did not exist at the merge base (contrast step 17) and the file is not on a reviewed-snapshot base (contrast step 11); step 15 covers an external origin.
10. `git mv` another file **and** edit it → still one R row, now `· adapted`, and the tooltip drops the `Identical to the origin` line; the diff shows only the edited lines, same title shape.
11. Mark a move row reviewed (inline `+`) → it moves to Reviewed and counts as one file. Edit it again → back to Needs Review with title `… (moved from … — last reviewed ↔ working tree)` and a diff of only the post-review edit.
12. The inline **Open File** action works on a move row (opens the new path).
13. Move a file with plain `mv` (unstaged) → old path shows as `D` plus an untracked `A` row at the new path. `git add -A`, refresh → the two rows collapse into one R row.
14. **Declared repo move git cannot detect**: leave that plain unstaged `mv` unstaged and hand-write a version 2 contract for the branch — `{"version": 2, "clusters": [], "moves": [{"path": "<new>", "from": "<old>", "origin": "repo"}]}` (path per the Clusters contract section; `clusters` is required even when empty, or the file fails with `"clusters" must be an array` before `moves` is read) → without staging anything, the `D` row at the old path disappears and the new path becomes a single R row rendering exactly as step 9, based on the old path's merge-base blob. Delete the declaration → the two rows return.
15. **Declared external move**: copy a file in from another project, `git hash-object -w` the donor's original (`git hash-object -w ../donor-app/src/telemetry/reporter.ts`), and declare it in a version 2 contract — `{"version": 2, "clusters": [], "moves": [{"path": "src/vendor/reporter.ts", "from": "../donor-app/src/telemetry/reporter.ts", "origin": "external", "donor": "donor-app", "baseBlob": "<that sha>", "note": "Swapped its logger for ours"}]}` → the row reads `src/vendor ← [donor-app] src/telemetry · adapted`, landing exactly on `ROW_DESCRIPTION_THRESHOLD` (48 chars); copy it byte-for-byte instead and the one-character-longer `· verbatim` tips it over, collapsing the origin to `src/vendor ← [donor-app] …/telemetry · verbatim`. The tooltip stacks the full origin, the note, and — when verbatim — `Identical to the origin`. The diff shows **only the adaptation**, not the whole file. The title names the donor path (`reporter.ts (moved from ../donor-app/src/telemetry/reporter.ts — merge base ↔ working tree)`) while the **left editor carries the file's own path** — an external path must never reach a base-document URI, the notes contract, or a ref. Add a base-side note and confirm `notes-<branch>.json` records the repo-relative path.
16. **Pruned `baseBlob`**: point that same `baseBlob` at a well-formed sha that is not in the object database → the row loses its classification word (`src/vendor ← [donor-app] src/telemetry`), the tooltip gains `Origin content is no longer available — showing the whole file.`, and the diff shows the whole file against an empty base. The title is unchanged.
17. **Move onto an existing path**: declare `{"version": 2, "clusters": [], "moves": [{"path": "<a path that exists at the merge base>", "from": "<another path you deleted>", "origin": "repo"}]}` → the destination is an R row whose diff is against **its own** merge-base blob (the overwrite reads as an edit to a file the reviewer already had), and the origin keeps its own `D` row, since its content is shown nowhere else.

Auto-review:

18. Set `deltaReview.autoReview.globs` (e.g. `["**/*.lock"]`) → matching files move into a collapsed **Auto** subgroup (⚙, count, flat with directory descriptions) first under Needs Review, in both layouts. No reload needed.
19. A file marked `linguist-generated` in `.gitattributes` lands in Auto even with empty globs.
20. Auto header `+` marks them all; they stay inspectable under Reviewed → Auto. Folder `+` does not touch auto files; Mark All still covers everything.
21. Flip `markAutomatically` on → next refresh self-marks auto files; edit one → it is silently re-marked with a fresh snapshot (stays under Reviewed → Auto, never resurfaces). Flip the setting off and edit it again → now it resurfaces under Needs Review → Auto with a delta diff.

Clusters:

22. No contract → no grouping button. Create a valid contract for the branch (run the `cluster-review` skill, or hand-write one at `.git/delta-review/clusters-<branch>.json`) → the group-by-cluster button appears without a manual refresh.
23. Group → clusters render in contract order with `n/m` counts and summary tooltips, showing only needs-review rows; **Unclustered** (warning-styled) after the clusters and collapsed **Auto** appear only while something in them needs review; an always-present **Reviewed** bucket renders last (`0` on a fresh branch); a cluster with no files in the change shows a message row. Mark a file → it moves into the Reviewed bucket, not marked in place with a `✓`; fully review a cluster → its header stays with `n/n` over a single `All files reviewed.` row.
24. Tree/list toggle still works inside clusters and the Reviewed bucket (per-cluster folder collapse, folder actions scoped to the cluster); cluster header `+`/`−` bulk-marks exactly that bucket. In the Reviewed bucket, folder `−` (tree mode) unmarks every visible child, auto files included — and with `markAutomatically` on, the auto file bounces back into Reviewed on the next refresh.
25. Both levers persist across reload. Flipping either lever changes no review state (`git ls-tree -r refs/review/<branch>` identical before/after).
26. Break the contract (e.g. `"version": 3`) → `⚠ Clusters contract: unsupported version 3 (extension supports 1 and 2)`, view falls back to ungrouped, **and every declared move reverts to plain rows** — an invalid contract costs moves as well as grouping. Grouping preference survives a later fix. Delete the contract → button and message disappear (moves revert the same way).
27. Throughout: `git status` in the test repo stays clean — the contract lives under `.git`.

Notes:

28. In a review diff, hover a right-side line and click the gutter `+` → the thread renders in place (Open), the REVIEW NOTES section lists it with the blue open icon, and the view badge counts it. `git status` stays clean; `.git/delta-review/notes-<branch>.json` and `refs/review-notes/<branch>` now exist.
29. Add a note on a **left** (base) side line → thread renders on the left editor with a `base` marker in REVIEW NOTES. Select a multi-line range first → the note spans the range.
30. Edit a reviewer turn (pencil on the comment) → Save persists, Cancel restores the original. Delete the only turn → the whole thread disappears (note gone from file and view); deleting one turn of a multi-turn thread keeps the rest.
31. Resolve from the thread title → green check in REVIEW NOTES, thread shows Resolved, badge drops. Unresolve → back to its derived status. The same three actions sit on the REVIEW NOTES row itself (hover icons and right-click: Resolve/Unresolve per status, Delete on any row behind a modal confirm) and work with no diff open.
32. Agent round-trip: hand-write `.git/delta-review/responses-<branch>.json` (`{"version":1,"responses":[{"noteId":"<id>","response":"…","at":"<UTC ISO-8601>"}]}`) → with **no manual refresh** the reply appears in the thread as Claude, the label flips to Addressed (yellow outline icon), and exactly one reply box appears. Type a reply and hit Reply & Reopen → Open again, reply box gone.
33. Anchor relocation: append a response entry whose `anchor` names another file/line with that line's exact text as `snapshot` → the note relocates there (a base-side note flips to the working side) and the REVIEW NOTES row follows. An anchor with a bad path shape, a missing file, or an out-of-range line is ignored — reply still shows, note stays put. `snapshot` is not validated: a wrong-but-in-range anchor still relocates the note and stores that snapshot verbatim.
34. Outdated: edit lines **above** a note → the thread shifts down/up, not outdated. Edit the noted line itself → `⚠` in REVIEW NOTES and a dimmed `line was: …` in the thread's first comment.
35. Base progression: with a base-side note on a file, mark the file reviewed → the base thread is recreated against the new base (the reviewed snapshot); turns and status untouched.
36. REVIEW NOTES navigation: click a note → the file's review diff opens with the cursor on the noted line and the thread expanded. A note on a file no longer in the review set opens the plain file; if the file is gone from disk too, it opens the note's recorded content (`delta-review-base` at its `contentBlob`) at the line it was written on. Delete a noted file that is still in the review set → the diff's right side is empty as always, but the thread renders on the **left** (base) side, where the content exists, at the last line if its anchor ran past the end. A file **added** on the branch and then deleted has no base blob either, so its diff is empty on both sides — the click skips the diff and opens the note's target document directly. Resolve from the row closes it in every case.
37. Selection sync: collapse the DELTA REVIEW groups (and its folders, in tree mode), then click a note row → the noted file's row is revealed and selected with its ancestors expanded, while focus stays on the diff. Those ancestors persist across reload exactly like a manual expand. Same in all four layouts (list/tree × grouped/ungrouped); a note on a file no longer in the review set leaves DELTA REVIEW untouched.
38. Clear Resolved (view title `$(clear-all)`) → resolved notes vanish from the file, the threads, and the ref, and `.git/delta-review/archive-<branch>.json` now holds them with `deletedAt`; open/addressed notes untouched; clicking again is a no-op (file mtime unchanged) and adds nothing to the archive.
39. Branch switch: `git switch` to another branch → that branch's own (empty or different) notes render; switch back → the originals return. Review marks and notes stay per-branch.
40. Corrupt files: garbage in the notes file → warning toast, notes unrendered, note actions refuse, and the extension **never rewrites the file**; restore it → everything returns. Garbage in the responses file → warning, notes still render (without replies), recovers when fixed. Each warning shows once, not per refresh.
41. Comments panel: open the built-in Comments panel → the same threads are listed there via the standard API; review tree, clusters, and auto-review behave exactly as before while notes exist.
42. Long note: add a note whose text runs 30+ lines → the whole note renders inline and the thread expands to fit it; no text is clipped, the thread carries no truncation link and no title-bar button beyond resolve/delete, and the only scrollbar is the editor's — scroll down to reach the file's lines below the widget. Edit that turn → the editor opens on the same full raw text (that editor scrolls once it passes ~500px; that is not clipping) and saving preserves it. Blank lines inside a note still separate paragraphs, and long lines still wrap rather than scrolling sideways.
