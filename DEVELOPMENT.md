...
35. REVIEW NOTES navigation: click a note → the file's review diff opens with the cursor on the noted line and the thread expanded. A note on a file no longer in the review set opens the plain file; if the file is gone from disk too, it opens the note's recorded content (`delta-review-base` at its `contentBlob`) at the line it was written on. Delete a noted file that is still in the review set → the diff's right side is empty as always, but the thread renders on the **left** (base) side, where the content exists, at the last line if its anchor ran past the end. A file **added** on the branch and then deleted has no base blob either, so its diff is empty on both sides — the click skips the diff and opens the note's target document directly. Resolve from the row closes it in every case.
36. Selection sync: collapse the DELTA REVIEW groups (and its folders, in tree mode), then click a note row → the noted file's row is revealed and selected with its ancestors expanded, while focus stays on the diff. Those ancestors persist across reload exactly like a manual expand. Same in all four layouts (list/tree × grouped/ungrouped); a note on a file no longer in the review set leaves DELTA REVIEW untouched.
37. Clear Resolved (view title `$(clear-all)`) → resolved notes vanish from the file, the threads, and the ref, and `.git/delta-review/archive-<branch>.json` now holds them with `deletedAt`; open/addressed notes untouched; clicking again is a no-op (file mtime unchanged) and adds nothing to the archive.
38. Branch switch: `git switch` to another branch → that branch's own (empty or different) notes render; switch back → the originals return. Review marks and notes stay per-branch.
39. Corrupt files: garbage in the notes file → warning toast, notes unrendered, note actions refuse, and the extension **never rewrites the file**; restore it → everything returns. Garbage in the responses file → warning, notes still render (without replies), recovers when fixed. Each warning shows once, not per refresh.
40. Comments panel: open the built-in Comments panel → the same threads are listed there via the standard API; review tree, clusters, and auto-review behave exactly as before while notes exist.
41. Long note: add a note whose text runs 30+ lines → the whole note renders inline and the thread expands to fit it; no text is clipped, the thread carries no truncation link and no title-bar button beyond resolve/delete, and the only scrollbar is the editor's — scroll down to reach the file's lines below the widget. Edit that turn → the editor opens on the same full raw text (that editor scrolls once it passes ~500px; that is not clipping) and saving preserves it. Blank lines inside a note still separate paragraphs, and long lines still wrap rather than scrolling sideways.
