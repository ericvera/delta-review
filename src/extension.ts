...
import { createReviewBaseContentProvider, createReviewBaseUri, REVIEW_BASE_SCHEME } from './contentProvider'
...
import { computeReviewModel, FileReviewStatus, ReviewFile, ReviewModel } from './model'
import { markReviewed, reviewRefForBranch, unmarkReviewed } from './reviewState'
import { collapseKeyFor, ReviewTreeProvider, ReviewTreeElement, ViewMode } from './treeProvider'
...
export const activate = async (context: vscode.ExtensionContext): Promise<void> => {
  ...
  const collapsed = new Set(context.workspaceState.get<string[]>(collapsedKey, []))
  ...
  void vscode.commands.executeCommand('setContext', 'deltaReview.viewMode', viewMode)
  ...
  const setViewMode = (mode: ViewMode): void => {
    ...
    void vscode.commands.executeCommand('setContext', 'deltaReview.viewMode', mode)
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('deltaReview.viewAsTree', () => setViewMode('tree')),
    vscode.commands.registerCommand('deltaReview.viewAsList', () => setViewMode('list'))
  )
  ...
  context.subscriptions.push(
    ...
    vscode.window.registerFileDecorationProvider(createReviewDecorationProvider())
  )
  ...
  const refresh = async (): Promise<void> => {
    ...
    if (git === undefined) {
      ...
      treeView.message = 'Open a folder inside a git repository to start reviewing.'
      statusBarItem.hide()
      return
    }
    const baseBranch =
      vscode.workspace.getConfiguration('deltaReview').get<string>('baseBranch') ?? 'main'
    try {
  ...
  const openDiff = async (file: ReviewFile): Promise<void> => {
    ...
    const baseLabel = file.diffBaseIsReviewedSnapshot ? 'last reviewed' : 'merge base'
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('deltaReview.markFileReviewed', async (element?: ReviewTreeElement) => {
      if (git === undefined || model === undefined || element?.kind !== 'file') {
        return
      }
    ...
    vscode.commands.registerCommand('deltaReview.unmarkFileReviewed', async (element?: ReviewTreeElement) => {
      if (git === undefined || model === undefined || element?.kind !== 'file') {
        return
      }
    ...
    vscode.commands.registerCommand('deltaReview.unmarkAllReviewed', async () => {
    ...
    vscode.commands.registerCommand('deltaReview.openFile', async (element?: ReviewTreeElement) => {
      if (git === undefined || element === undefined || element.kind !== 'file' || element.file.deleted) {
        return
      }
      await vscode.window.showTextDocument(vscode.Uri.file(join(git.repoRoot, element.file.path)))
    }),

    vscode.commands.registerCommand('deltaReview.markFolderReviewed', async (element?: ReviewTreeElement) => {
      if (git === undefined || model === undefined || element?.kind !== 'folder') {
        return
      }
      const paths = model.files
        .filter(
          (file) =>
            file.status === FileReviewStatus.NeedsReview && file.path.startsWith(`${element.path}/`)
        )
        .map((file) => file.path)
      if (paths.length === 0) {
        return
    ...
    vscode.commands.registerCommand('deltaReview.unmarkFolderReviewed', async (element?: ReviewTreeElement) => {
      if (git === undefined || model === undefined || element?.kind !== 'folder') {
        return
      }
      const paths = model.files
        .filter(
          (file) =>
            file.status === FileReviewStatus.Reviewed && file.path.startsWith(`${element.path}/`)
        )
        .map((file) => file.path)
      if (paths.length === 0) {
        return
    ...
    vscode.commands.registerCommand('deltaReview.clearReviewState', async () => {
      if (git === undefined || model === undefined) {
        return
      }
  ...
  if (gitApi !== undefined) {
    ...
    const syncActiveRepo = (): void => {
      ...
      const active = repositories.find((repo) => repo.ui.selected) ?? repositories[0]
    ...
    if (workspaceFolder !== undefined) {
      try {
        ...
        const repoRoot = (await candidate.run(['rev-parse', '--show-toplevel'])).trim()
