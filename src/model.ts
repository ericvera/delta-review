...
export const computeReviewModel = async (git: Git, baseBranch: string): Promise<ReviewModel> => {
  ...
  const trackedOutput = await git.run(['diff', '--name-only', '--no-renames', '-z', mergeBase])
  const untrackedOutput = await git.run(['ls-files', '--others', '--exclude-standard', '-z'])
  const paths = [
    ...new Set([...splitNulTerminated(trackedOutput), ...splitNulTerminated(untrackedOutput)]),
  ].sort()

  const reviewState = await readReviewState(git, branch)
  const baseBlobs = parseLsTreeOutput(await git.run(['ls-tree', '-r', '-z', mergeBase]))
  const sentinelSha = (
    await git.run(['hash-object', '--stdin'], { stdin: DELETED_SENTINEL_CONTENT })
  ).trim()
  ...
  if (existingPaths.length > 0) {
    ...
    existingPaths.forEach((path, index) => currentShaByPath.set(path, shas[index]))
  }
  ...
  const files = paths.map((path): ReviewFile => {
    ...
    const snapshotUsable = reviewedSha !== undefined && reviewedSha !== sentinelSha
    const useSnapshotBase = !reviewed && snapshotUsable
    return {
      path,
      status: reviewed ? FileReviewStatus.Reviewed : FileReviewStatus.NeedsReview,
      deleted,
