...
name: review-notes
description: >-
  Address the reviewer's inline review notes written in the Delta Review VS
  Code extension: read the notes contract, fix the code each note asks about,
  and reply through the responses contract file. Use when the user asks to
  address my review notes, handle my diff comments, respond to review notes,
  or otherwise act on Delta Review notes. Requires a git repository with a
  feature branch.
---

# Review Notes

Act on the inline review notes a human wrote in the Delta Review VS Code extension. Two JSON files per branch make up the contract:
...
- `status`, `contentBlob`, `appliedAnchorAt` — extension bookkeeping. `status` matters only as the resolved-check below; everything else you can ignore.
...
If the notes file is missing, or parses with an empty `notes` array, report "no review notes on this branch" and stop. If it exists but is not valid JSON, report that to the user and stop — never repair or rewrite the notes file.

### 3. Compute the work set

Read the responses file too (missing file = no responses yet; if it exists but is corrupt, stop and tell the user rather than clobbering it). A note is **actionable** iff both:
...
### 4. Address each actionable note
...
Append one entry per note you addressed:
